...
-- Для SQLite (используется в Django по умолчанию)
-- =====================================================

-- Удаление существующих таблиц (если нужно пересоздать БД)
-- ВНИМАНИЕ: Раскомментируйте только если нужно удалить существующие данные!
-- DROP TABLE IF EXISTS catalog_favorite;
-- DROP TABLE IF EXISTS catalog_review;
-- DROP TABLE IF EXISTS catalog_notification;
-- DROP TABLE IF EXISTS catalog_message;
-- DROP TABLE IF EXISTS catalog_rental;
...
CREATE TABLE IF NOT EXISTS catalog_profile (
    ...
    rating DECIMAL(3,2) NOT NULL DEFAULT 5.0 CHECK(rating >= 1.0 AND rating <= 5.0),
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES auth_user(id) ON DELETE CASCADE
);

-- Индексы для таблицы catalog_profile
CREATE INDEX IF NOT EXISTS idx_catalog_profile_user_id ON catalog_profile(user_id);
CREATE INDEX IF NOT EXISTS idx_catalog_profile_role ON catalog_profile(role);
CREATE INDEX IF NOT EXISTS idx_catalog_profile_rating ON catalog_profile(rating);
...
-- Индексы для таблицы catalog_category
CREATE INDEX IF NOT EXISTS idx_catalog_category_parent_id ON catalog_category(parent_id);
CREATE INDEX IF NOT EXISTS idx_catalog_category_name ON catalog_category(name);
...
-- Индексы для таблицы catalog_item
CREATE INDEX IF NOT EXISTS idx_catalog_item_category_id ON catalog_item(category_id);
CREATE INDEX IF NOT EXISTS idx_catalog_item_subcategory_id ON catalog_item(subcategory_id);
...
CREATE INDEX IF NOT EXISTS idx_catalog_item_created_at ON catalog_item(created_at);
...
-- Индексы для таблицы catalog_rental
CREATE INDEX IF NOT EXISTS idx_catalog_rental_item_id ON catalog_rental(item_id);
CREATE INDEX IF NOT EXISTS idx_catalog_rental_renter_id ON catalog_rental(renter_id);
CREATE INDEX IF NOT EXISTS idx_catalog_rental_status ON catalog_rental(status);
CREATE INDEX IF NOT EXISTS idx_catalog_rental_start_date ON catalog_rental(start_date);
CREATE INDEX IF NOT EXISTS idx_catalog_rental_created_at ON catalog_rental(created_at);
...
-- Индексы для таблицы catalog_message
CREATE INDEX IF NOT EXISTS idx_catalog_message_sender_id ON catalog_message(sender_id);
CREATE INDEX IF NOT EXISTS idx_catalog_message_recipient_id ON catalog_message(recipient_id);
CREATE INDEX IF NOT EXISTS idx_catalog_message_is_read ON catalog_message(is_read);
CREATE INDEX IF NOT EXISTS idx_catalog_message_created_at ON catalog_message(created_at);
...
CREATE TABLE IF NOT EXISTS catalog_notification (
    ...
    notification_type VARCHAR(20) NOT NULL CHECK(notification_type IN ('rental_request', 'rental_confirmed', 'rental_cancelled', 'message', 'rental_completed')),
    title VARCHAR(200) NOT NULL,
    message TEXT NOT NULL,
    is_read BOOLEAN NOT NULL DEFAULT 0,
    related_item_id INTEGER NULL,
    related_rental_id INTEGER NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES auth_user(id) ON DELETE CASCADE,
...
-- Индексы для таблицы catalog_notification
CREATE INDEX IF NOT EXISTS idx_catalog_notification_user_id ON catalog_notification(user_id);
CREATE INDEX IF NOT EXISTS idx_catalog_notification_is_read ON catalog_notification(is_read);
CREATE INDEX IF NOT EXISTS idx_catalog_notification_created_at ON catalog_notification(created_at);
CREATE INDEX IF NOT EXISTS idx_catalog_notification_type ON catalog_notification(notification_type);
...
-- Индексы для таблицы catalog_review
CREATE INDEX IF NOT EXISTS idx_catalog_review_reviewer_id ON catalog_review(reviewer_id);
CREATE INDEX IF NOT EXISTS idx_catalog_review_reviewed_user_id ON catalog_review(reviewed_user_id);
CREATE INDEX IF NOT EXISTS idx_catalog_review_item_id ON catalog_review(item_id);
CREATE INDEX IF NOT EXISTS idx_catalog_review_rental_id ON catalog_review(rental_id);
CREATE INDEX IF NOT EXISTS idx_catalog_review_rating ON catalog_review(rating);
CREATE INDEX IF NOT EXISTS idx_catalog_review_created_at ON catalog_review(created_at);
...
-- Индексы для таблицы catalog_favorite
CREATE INDEX IF NOT EXISTS idx_catalog_favorite_user_id ON catalog_favorite(user_id);
CREATE INDEX IF NOT EXISTS idx_catalog_favorite_item_id ON catalog_favorite(item_id);
CREATE INDEX IF NOT EXISTS idx_catalog_favorite_created_at ON catalog_favorite(created_at);
...
-- Вставка категорий
INSERT OR IGNORE INTO catalog_category (id, name, description, icon) VALUES
...
INSERT OR IGNORE INTO catalog_category (id, name, description, parent_id, icon) VALUES
...
-- Вставка пользователей (пароли захешированы, пример: 'pbkdf2_sha256$...')
-- В реальном проекте пароли должны быть правильно захешированы через Django
INSERT OR IGNORE INTO auth_user (id, username, email, first_name, last_name, password, is_active, date_joined) VALUES
...
-- Вставка профилей
INSERT OR IGNORE INTO catalog_profile (user_id, phone, role, rating) VALUES
...
-- Вставка товаров
INSERT OR IGNORE INTO catalog_item (id, title, description, price_per_day, deposit, category_id, subcategory_id, owner_id, status) VALUES
...
-- Вставка аренд
INSERT OR IGNORE INTO catalog_rental (item_id, renter_id, start_date, end_date, total_price, status) VALUES
...
-- Вставка сообщений
INSERT OR IGNORE INTO catalog_message (sender_id, recipient_id, content, is_read) VALUES
...
-- Вставка уведомлений
INSERT OR IGNORE INTO catalog_notification (user_id, notification_type, title, message, related_item_id, related_rental_id, is_read) VALUES
...
-- Вставка отзывов
INSERT OR IGNORE INTO catalog_review (reviewer_id, reviewed_user_id, item_id, rental_id, rating, comment) VALUES
...
-- Вставка избранного
INSERT OR IGNORE INTO catalog_favorite (user_id, item_id) VALUES
(3, 2),
(3, 4),
(4, 1),
(5, 3),
